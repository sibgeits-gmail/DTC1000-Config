...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent)
{
    ...
    centralWidget->setMinimumWidth(750);
    centralWidget->setMinimumHeight(950);
    ...
    map_table_2["1020"] = QPoint(0, 2);
    map_table_2["1021"] = QPoint(1, 2);
    map_table_2["1023"] = QPoint(2, 2);
    //map_table_2["102A"] = QPoint(3, 2);       // Вручную указываются ячейки в обработчики для этого регистра (2 ячейки для 2ух битов выходов)
    map_table_2["1071"] = QPoint(5, 2);
    map_table_2["1072"] = QPoint(6, 2);
    map_table_2["1073"] = QPoint(7, 2);
    map_table_2["1074"] = QPoint(8, 2);
    map_table_2["1075"] = QPoint(9, 2);
    map_table_2["1076"] = QPoint(10, 2);
    ...
    tableRegLeft = new QTableWidget(this);
    tableRegLeft->setColumnCount(3);
    tableRegLeft->setHorizontalHeaderLabels({"Address", "Name", "Data"});
    tableRegLeft->verticalHeader()->setVisible(false);
    tableRegLeft->horizontalHeader()->setStretchLastSection(true);
    tableRegLeft->setColumnWidth(0, 80);
    tableRegLeft->setColumnWidth(1, 200);
    tableRegLeft->setColumnWidth(2, 80);
    tableRegLeft->setMinimumHeight(350);

    for (int row = 0; row < 10; row++){
        tableRegLeft->insertRow(row);
        tableRegLeft->setItem(row, 0, new QTableWidgetItem(registers[row]));
        tableRegLeft->item(row,0)->setTextAlignment(Qt::AlignCenter);
        tableRegLeft->setItem(row, 1, new QTableWidgetItem(regNames[row]));
    }

    tableRegRight = new QTableWidget(this);
    tableRegRight->setColumnCount(3);
    tableRegRight->setHorizontalHeaderLabels({"Address", "Name", "Data"});
    tableRegRight->verticalHeader()->setVisible(false);
    tableRegRight->horizontalHeader()->setStretchLastSection(true);
    tableRegRight->setColumnWidth(0, 80);
    tableRegRight->setColumnWidth(1, 200);
    tableRegRight->setColumnWidth(2, 80);
    tableRegRight->setMinimumHeight(380);

    for (int row = 0; row < 11; row++){
        tableRegRight->insertRow(row);
        tableRegRight->setItem(row, 0, new QTableWidgetItem(registers[row+10]));
        tableRegRight->item(row,0)->setTextAlignment(Qt::AlignCenter);
        tableRegRight->setItem(row, 1, new QTableWidgetItem(regNames[row+10]));
    }

    auto* layoutTables = new QHBoxLayout();
    layoutTables->addWidget(tableRegLeft);
    layoutTables->addWidget(tableRegRight);
    ...
    mainLayout->addLayout(layoutTables);
    ...
    connect(tableRegLeft, &QTableWidget::cellChanged,
            this, &MainWindow::handleCellChangedLeft);
    connect(tableRegRight, &QTableWidget::cellChanged,
            this, &MainWindow::handleCellChangedRight);
...
{
    tableRegLeft->blockSignals(true);   // Отключим сигналы таблиц, чтобы изменение значений в ячейках не вызвало отправку данных
    tableRegRight->blockSignals(true);
    ...
    if (map_table_1.contains(addr)){    // Отдельный обработчик для левой таблицы
        int row = map_table_1[addr].x();
        int col = map_table_1[addr].y();
        if (!isEditing(tableRegLeft, row, col))
            tableRegLeft->setItem(row, col, new QTableWidgetItem(QString::number(val)));
    }
    else if (map_table_2.contains(addr)){   // Отдельный обработчик для правой таблицы
        int row = map_table_2[addr].x();
        int col = map_table_2[addr].y();
        if (!isEditing(tableRegRight, row, col))
            tableRegRight->setItem(row, col, new QTableWidgetItem(QString::number(val)));
    }
    else if (addr == "102A"){   // Отдельный обработчик для этого регистра, потому что в нём 2 параметра
        tableRegRight->setItem(3, 2, new QTableWidgetItem(QString::number((val >> 5) & 1)));
        tableRegRight->setItem(4, 2, new QTableWidgetItem(QString::number((val >> 6) & 1)));
    }

    tableRegLeft->blockSignals(false);
    tableRegRight->blockSignals(false);
}

void MainWindow::handleCellChangedLeft(int row, int column) // Записать в регистр после ручного ввода в таблицу
{
    QTableWidgetItem *itemVal = tableRegLeft->item(row, column);
    QTableWidgetItem *itemAddr = tableRegLeft->item(row, 0);
...
void MainWindow::handleCellChangedRight(int row, int column)    // Записать в регистр после ручного ввода в таблицу
{
    QTableWidgetItem *itemVal = tableRegRight->item(row, column);
    QTableWidgetItem *itemAddr = tableRegRight->item(row, 0);
